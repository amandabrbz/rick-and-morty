...
Modal.Image = function ModalImage({ background, children }) {
  ...
  return (
    <section className="modal-content__image" style={applyBackground}>
      {children}
      <button
      ...
      >
        <span className="modal-content__close--text">Close</span>{' '}
        <span className="modal-content__close--symbol">&times;</span>
      </button>
    </section>
  )
}
...
Modal.Content = function ModalContent({ data }) {
  return (
    <section className="modal-content__description">
      <article className="modal-content__description--wrapper">
        <h2 className="modal-content__description--title">About</h2>
        <p className="modal-content__description--description">
          {data.name} is a {data.species} from the gender {data.gender} and it's
          currrent status is {data.status}.
        </p>
      </article>
      <article className="modal-content__description--wrapper">
        ...
        <p className="modal-content__description--obs">
          <img
            src={Icon}
            alt="icon representing population"
            role="presentation"
          />
